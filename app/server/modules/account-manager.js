...
exports.updateWeight = function (user, questionId, weight, callback) {
    var tableName =  user + "_question_weights";
    var query = "UPDATE " + tableName + " SET weight=$1 WHERE question_id=$2;";
    client.query(query, [weight, questionId], (err, result) => {
        if (err) {
            callback(false, "Error updating weights");
        } else {
            callback(true, "Question weight updated");
        }
    });
};
